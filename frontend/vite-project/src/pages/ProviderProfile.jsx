...
import { Briefcase, MapPin, Star, Award, Edit2, MessageSquare, ShieldCheck, Sparkles, Loader2, Phone, Mail, CheckCircle2, Camera } from "lucide-react";
...
function ProviderProfile() {
   ...
   // Tier based on review count
   const getTier = (count) => {
      if (count === 0) return { label: "Newcomer", emoji: "🥉", color: "text-slate-400", bg: "bg-slate-100", border: "border-slate-200", next: "Get your first review to level up!" };
      if (count <= 5) return { label: "Rising Pro", emoji: "🥈", color: "text-amber-500", bg: "bg-amber-50", border: "border-amber-200", next: `${6 - count} more reviews to reach Experienced` };
      if (count <= 15) return { label: "Experienced", emoji: "🥇", color: "text-primary-500", bg: "bg-primary-50", border: "border-primary-200", next: `${16 - count} more reviews to reach Top Rated` };
      return { label: "Top Rated", emoji: "🏆", color: "text-emerald-500", bg: "bg-emerald-50", border: "border-emerald-300", next: "You've reached the highest tier!" };
   };
               ...
               <div className="lg:col-span-4 space-y-8">
                  <div className="bg-white rounded-[3rem] border border-slate-100 shadow-premium overflow-hidden transition-all duration-500">
                     ...
                     <div className="p-8 space-y-6">
                        ...
                        >
                           <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">Current Tier</p>
                           <div className="flex items-center gap-3 mb-2">
                              <span className="text-3xl">{tier.emoji}</span>
                              <span className={`text-lg font-black uppercase tracking-wide ${tier.color}`}>{tier.label}</span>
                           </div>
                           <p className="text-[10px] font-bold text-slate-400 leading-relaxed">{tier.next}</p>
                        </motion.div>
                     </div>
                  </div>
               </div>
                        ...
                        >
                           ...
                           <div className="flex items-center gap-6">
                              <div className="w-20 h-20 rounded-3xl bg-slate-100 flex items-center justify-center overflow-hidden border border-slate-200">
                                 {formData.profilePicture ? (
                                    ...
                                    <span className="text-slate-500 font-black text-2xl">{profile.name.charAt(0)}</span>
                                 )}
                              </div>
                              <button
                                 type="button"
                           ...
                           <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                              <div className="space-y-2">
                                 ...
                                 <input type="text" name="name" value={formData.name} onChange={handleInputChange} className="w-full px-6 py-4 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-bold text-slate-900 focus:outline-none focus:ring-4 focus:ring-primary-500/10 focus:border-primary-500 transition-all placeholder:text-slate-300 shadow-inner" />
                              </div>
                              <div className="space-y-2">
                                 ...
                                 <input type="tel" name="phone" value={formData.phone} onChange={handleInputChange} className="w-full px-6 py-4 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-bold text-slate-900 focus:outline-none focus:ring-4 focus:ring-primary-500/10 focus:border-primary-500 transition-all shadow-inner" />
                              </div>
                              <div className="space-y-2">
                                 ...
                                 <input type="text" name="city" value={formData.city} onChange={handleInputChange} className="w-full px-6 py-4 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-bold text-slate-900 focus:outline-none focus:ring-4 focus:ring-primary-500/10 focus:border-primary-500 transition-all shadow-inner" />
                              </div>
                              <div className="space-y-2">
                                 ...
                                 <input type="text" name="services" value={formData.services} onChange={handleInputChange} className="w-full px-6 py-4 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-bold text-slate-900 focus:outline-none focus:ring-4 focus:ring-primary-500/10 focus:border-primary-500 transition-all shadow-inner" />
                              </div>
                              <div className="space-y-2">
                                 ...
                                 <input type="text" name="state" value={formData.state} onChange={handleInputChange} className="w-full px-6 py-4 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-bold text-slate-900 focus:outline-none focus:ring-4 focus:ring-primary-500/10 focus:border-primary-500 transition-all shadow-inner" />
                              </div>
                              <div className="space-y-2">
                                 ...
                                 <input type="number" min="0" name="experienceYears" value={formData.experienceYears} onChange={handleInputChange} className="w-full px-6 py-4 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-bold text-slate-900 focus:outline-none focus:ring-4 focus:ring-primary-500/10 focus:border-primary-500 transition-all shadow-inner" />
                              </div>
                           </div>
                           <div className="space-y-2">
                              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Narrative Bio</label>
                              <textarea name="description" value={formData.description} onChange={handleInputChange} rows={4} className="w-full px-6 py-4 bg-slate-50 border border-slate-100 rounded-[2rem] text-sm font-medium focus:outline-none focus:ring-4 focus:ring-primary-500/10 focus:border-primary-500 transition-all resize-none shadow-inner" />
                           </div>
